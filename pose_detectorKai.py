...
class PoseDetector(object):
    ...
    def compute_peaks_from_heatmaps(self, heatmaps):
        ...
        if xp == np:
            ...
            for i , heatmap in enumerate(heatmaps):
                ...
                peaks = zip(xp.nonzero(peaks_binary)[1], xp.nonzero(peaks_binary)[0])
    ...
    def compute_candidate_connections_greedy(self, paf, cand_a, cand_b, img_len, params):
        ...
        for index_a, joint_a in enumerate(cand_a):
            for index_b, joint_b in enumerate(cand_b):
                ...
                integ_value_with_dist_prior = integ_value + min(params['length_penalty_ratio'] * img_len / vec_len - 1, 0)
    ...
    def grouping_key_points(self, all_connections, candidate_peaks, params):
        ...
        for connection_category_index in range(len(params['limbs_point'])):
            ...
            joint_category_a_index, joint_category_b_index = params['limbs_point'][connection_category_index] 
            ...
            for connection_index, _ in enumerate(all_connections[connection_category_index]):
                ...
                if joint_found_cnt == 1:
                    ...
                    if len(np.nonzero(membership == 2)[0]) == 0: # merge two subsets when no duplication
                        ...
                        found_subset_1[-2:] += all_connections[connection_category_index][connection_index][2] 
                ...
                elif joint_found_cnt == 0 and connection_category_index < 17: 
                    row = -1 * np.ones(20)
        ...
        for i, (limb, color) in enumerate(zip(params['limbs_point'], limb_colors)):
            if i != 9 and i != 13:  # don't show ear-shoulder connection
                ...
                if np.all(pose[limb_ind][:, 2] != 0):
                    ...
                    #attend me
                    for x in list(joint1):
                        f.write(str(x) + ',')
                        
                    f.write("\n")
                    for y in list(joint2):
                        f.write(str(y) + ',')
